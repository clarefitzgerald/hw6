...
fun typeof (VAR(x), Delta, Gamma) = 
   let val tau = find(x, Gamma) (* (handle NotFound _ => raise TypeError ("Undeclared Variable Name")) *)
    in tau
    end
| typeof (SET (x, e), Delta, Gamma) =
    let  
    val tau2 = (find(x, Gamma) handle NotFound _ => raise TypeError ("Undeclared Variable Name"))
    val tau1 = typeof(e, Delta, Gamma)
    in if eqType(tau1, tau2)
      then tau1
      else raise TypeError "Expression and Variable value must have the same type"
    end
| typeof (LITERAL v, Delta, Gamma) = 
  let 
    fun literaltype(BOOLV b) = booltype
    | literaltype(NUM n) = inttype
    | literaltype(SYM s) = symtype
    | literaltype(NIL) = unittype
    | literaltype(_) = raise TypeError "Type not implemented"
    val tau = literaltype(v)
  in tau
  end
| typeof (IFX (e1, e2, e3), Delta, Gamma) = 
  let val tau1 = typeof(e1, Delta, Gamma)
      val () = 
        ...
        then ()
        else raise TypeError "First arg must be boolean"
    val tau2 = typeof(e2, Delta, Gamma)
    val tau3 = typeof(e3, Delta, Gamma)
    val () = 
      if eqType(tau2, tau3) then ()
      else raise TypeError "Then and Else must have the same type"
  in tau2
  end
...
| typeof (BEGIN (es), Delta, Gamma) = 
  let val bodytypes = map (fn (e:exp) => typeof(e, Delta, Gamma)) es
...
| typeof _ = raise TypeError "Left as Exercise"
fun typdef (EXP e, Delta, Gamma) =
  let val tau = typeof(e, Delta, Gamma)
  in (Gamma, typeString tau)  
  end
(*| typdef (VAL (x, e), Delta, Gamma) =
     if not (isbound (x, Gamma)) then
        let val tau  = typeof (e, Delta, Gamma)
        in  (bind (x, tau, Gamma), Gamma, typeString tau)
        end
    else
        let val tau' = find (x, Gamma)
            val tau  = typeof (e, Gamma)
        in  if eqType (tau, tau') then
            ((Delta, Gamma), typeString tau)
             else
...
(* type declarations forliterallir consistency checking *)
